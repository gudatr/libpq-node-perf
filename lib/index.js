...
class Postgres {
    constructor(config) {
        ...
        if (!config.socket) {
            this.connectionString = `postgresql://${config.user}:${(_a = config.password) !== null && _a !== void 0 ? _a : ''}@${config.host}:${config.port}/${config.database}`;
        }
        else {
...
class PostgresClient extends Libpq {
    ...
    connect(connectionString) {
        this.names = [];
        this.types = [];
        if (!this.$connectSync(connectionString) || !this.$setNonBlocking(1))
